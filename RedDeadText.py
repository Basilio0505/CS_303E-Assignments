...
'''Will be used to randomly choose who gets hit by enemies in combat'''         #Two types of comments pt1
import random

'''Slows down the amount of text the player gets by having them hit a key'''
def hitEnter():          #Function Definition and Function Call                  Two types of comments pt2
    print("")            #print() statement
    input("[Hit Enter] ")

'''Shop for the user to spend his currency to get new weapons and recover health'''
def shop(player):
    while True:        #While Loop
        try:           #Try and Except Block
            ...
            if item ==0:                     #If Statement
                ...
                item = "invaild"         #Assignment Statement
                item = int(item)
            while True:
                try:
...
def travel(world, index):    #Function Definition with Parameters and Function Call
        while index < 0 or index >= len(world):
            print(world)
            action = input("Where would you like to go next? -->").lower()
            for a in range(0, len(world)):        #For Loop
                if world[a] == action:
                    index = a
                    break
...
def fight(friends, number = 1):  #Function Definition with Default Parameters and Function Call
    enemies = {}            #Dictionary
...
def bossFight(friends):
    life = 30
    while life > 0:
        if len(friends) == 0:
            print("EVERYONE IS DEAD. GAME OVER.")
            break
        rnum = random.randint(0, len(friends) - 1)
        print("Cactus Dan hit " + friends[rnum].getName())
        friends[rnum].hit(4)
        if friends[rnum].getHealth() <= 0:
            print("Oh NO! "+friends[rnum].getName()+" is dead!!")
            friends.remove(friends[rnum])
        ...
        for z in friends:
            if life <= 0:
                print("CACTUS DAN IS DEAD! YOU WIN!")
                break
            print(str(z.getName()) + " hit Cactus Dan")
            life -= z.weapondamage()                                     #Use of +=, -=, etc.
            print("Cactus Dan's health is at " + str(life))
            hitEnter()
...
def main():
    ...
    hitEnter()
    name = input("?:\"Hello new sheriff what is your name\"--> ")
    ...
    world = ["saloon", "bank", "mansion", "store"]  # List for environment
    posse = [player, carson]                        # List of allies
    index = len(world)
    index = travel(world, index)   #Function Definition with Parameters and Function Call
    hitEnter()
    ...
    while loop == 100:
        ...
        while loop == 0:
            ...
            while True:
                try:
                    ask1 = int(input("[1](Buy them a couple drinks)/[2](You remember seeing her missing poster "
            ...
            while True:
                try:
                    ask2 = int(input("[1](Call him out)/[2](Leave him be)    Enter Number --> "))
                    if ask2 == 1:
        ...
        while loop == 1:
            ...
            while True:
                try:
                    ask1 = int(input("[1](Jump behind cover)/[2](Push Billy to cover taking some damage)"
                                     "\nEnter number --> "))
                    if ask1 == 1:
            ...
            while True:
                try:
                    ask2 = int(input("[1](Save the workers first)/[2](Try to fight the two robbers by yourself while "
        ...
        while loop == 2:
            ...
            print("The large mansion located on the outskirts of town once belong to a rich oil tycoon. It has since \n"
                  "then been abandoned and is rundown. All it's windows are smashed all the paint has peeled off.")
            hitEnter()
            while True:
                try:
                    again = int(input("This is the Final battle are you sure you are ready to face Cactus Dan?!\n"
                                  "[1]yes/[2]no    Enter number--> "))
                    if again == 1:
                        print("Cactus Dan emerges to greet you.")
                        print("Cactus Dan is dressed in typical rugged outlaw clothing. He has a thick handlebar mustache\n"
                              "and long bushy bread. He has long black hair and stuck to his cowbay hat is the siganture\n"
                              "piece of cactus He wears an eye patch and harbors many scars from many successful shootouts")
                        print("Cactus Dan: \"Listen here boy. You messin with my boys has pissed me off and imma enjoying\n"
                              " killing you.\"")
                        hitEnter()
                    elif again == 2:
                        index = travel(world, len(world))
            ...
            if player.getHealth() <= 0:
                ...
                break
            hitEnter()
            print("You see Cactus Dan's lifeless body lying on the ground. You have successfully saved the town from\n"
                  "this evil menace.")
